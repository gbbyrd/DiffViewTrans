...
class UViT(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        context_token = torch.cat((time_token,context_token),dim=1)
        context_token = context_token + self.pos_embed
        x = torch.cat((time_token, x), dim=1)
        x = x + self.pos_embed

        skips = []
        for blk in self.in_blocks:
            x = blk(x,context_token)
            skips.append(x)

        x = self.mid_block(x)

        for blk in self.out_blocks:
            x = blk(x, context_token,skips.pop())
