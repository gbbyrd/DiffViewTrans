...
class Encoder_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip=False, use_checkpoint=False,patch_size=4,in_chans=3):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn1 = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale)
        ...
        self.skip_linear = nn.Linear(2 * dim, dim) if skip else None
        self.use_checkpoint = use_checkpoint
        self.condition_embed = PatchEmbed(patch_size=patch_size, in_chans=in_chans, embed_dim=dim)

    def forward(self, x,condition,time_token,pos,skip=None):
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(self._forward, x, condition,time_token,pos,skip)
        else:
            return self._forward(x,condition,time_token,pos,skip)

    def _forward(self, x, condition,time_token,pos,skip=None):
...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.encoder = nn.ModuleList([
            Encoder_Block(
                ...
                norm_layer=norm_layer, use_checkpoint=use_checkpoint,skip=False,patch_size=patch_size,in_chans=in_chans)
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        for blk in self.encoder:
            x = blk(x,context,time_token,self.pos_embed)
            skips.append(x)
        last_output = x
        index = len(skips)-1
        x = self.mid_block(x)
