...
class Decoder_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip=False, use_checkpoint=False,condition_SA=True):
        super().__init__()
        self.norm1 = norm_layer(dim)
        ...
        self.skip_linear = nn.Linear(2 * dim, dim) if skip else None
        self.use_checkpoint = use_checkpoint
        self.sa = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale) if condition_SA else None
        self.norm_cond = norm_layer(dim) if condition_SA else None
    ...
    def _forward(self, x, condition,skip=None):
        ...
        if self.sa is not None:
            condition = condition + self.sa(self.norm_cond(condition))
...
class UViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
        ...
        self.decoder = nn.ModuleList([
            Decoder_Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                norm_layer=norm_layer, skip=skip, use_checkpoint=use_checkpoint,condition_SA=False)
            for _ in range(depth // 2)])
