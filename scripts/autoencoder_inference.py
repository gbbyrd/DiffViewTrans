...
if __name__ == "__main__":
    ...
    while 1:
        ...
        # normalize image
        normalized_img = transform(img).unsqueeze(0)

        # pass image through network
        output = model(normalized_img)

        recreated_img = output[0].squeeze().detach().cpu().numpy()
        recreated_img = np.transpose(recreated_img, (1, 2, 0))
        img = normalized_img.squeeze().detach().cpu().numpy()
        img = np.transpose(img, (1, 2, 0))
        # # normalize recreated img
        # recreated_img = (recreated_img + 1) * 127.5
        full_img = np.concatenate((img, recreated_img), axis=1)
