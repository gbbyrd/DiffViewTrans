...
def preprocess_frame_data(aerial_rgb, aerial_depth, translation_label, sensor_params):
    ...
    for key in translation_label:
        min_val = sensor_params['initial_spawn_limits'][key][0]
        max_val = sensor_params['initial_spawn_limits'][key][1]
        if min_val != max_val:
            translation_label[key] = (translation_label[key] - min_val) / (max_val - min_val)
